...
const VERSION='5';
const assets=new Map(await Promise.all(['index.html','app.js','style.css','rules.mjs','live-connection.mjs'].map(async name=>[name,await readFile(new URL(name,import.meta.url))])));
...
function send(p,type,data){
  if(type==='chat'){
    data={...data,id:randomUUID()};p.chatHistory??=[];p.chatHistory.push(data);
    if(p.chatHistory.length>250)p.chatHistory.shift();
  }
  for(const stream of p.streams){
    if(stream.destroyed||stream.writableEnded){p.streams.delete(stream);stream.destroy?.();if(!p.streams.size)p.lastSeen=Date.now();continue;}
    try{stream.write(`event: ${type}\ndata: ${JSON.stringify(data)}\n\n`);}catch{p.streams.delete(stream);stream.destroy?.();if(!p.streams.size)p.lastSeen=Date.now();}
  }
}
...
export function snapshot(r,p,now=Date.now()){
  return {code:r.code,host:r.host,players:r.players.map(q=>({...playerInfo(q),spectator:isSpectator(r,q)})),phase:r.phase,drawer:r.drawer,
    ...
    finishReason:r.finishReason,settingsRevision:r.settingsRevision,wordCount:r.words.length,customWords:r.host===p.id?r.customWords:undefined};
}
...
function next(r){
  ...
  if(!r.queue.length){
    ...
    r.round++;r.queue=r.players.map(p=>p.id);r.drawnThisRound=new Set();
  }
  r.drawer=r.queue.shift();r.drawnThisRound.add(r.drawer);r.turn++;r.phase='choose';r.word=null;r.strokes=[];r.results=[];
...
function reveal(r,reason=''){
  ...
  r.settled=true;
  const times=[...r.guesses.values()],order=[...r.guesses.keys()];
  const awards=new Map(r.eligible.map(id=>[id,r.guesses.has(id)?guessPoints(r.guesses.get(id),order.indexOf(id)+1,r.duration*1000):0]));
...
function removePlayer(r,p,explicit=false){
  if(!r.players.includes(p))return;
  p.detachedAt=Date.now();p.explicitLeave=explicit;
  r.players=r.players.filter(q=>q!==p);for(const stream of p.streams)stream.end?.();p.streams.clear();
  if(!r.players.length){if(explicit)rooms.delete(r.code);else r.emptySince=Date.now();return;}
...
export function tick(now=Date.now()){
  for(const r of rooms.values()){
    // Temporarily detached players can resume for two minutes without losing their identity.
    if(r.emptySince&&now-r.emptySince>120000){rooms.delete(r.code);continue;}
    for(const p of [...r.players])if(!p.streams.size&&now-p.lastSeen>15000)removePlayer(r,p);
...
export async function handleRequest(req,res){
  ...
  try{
    ...
    if(url.pathname==='/events'){
      ...
      res.writeHead(200,{'Content-Type':'text/event-stream; charset=utf-8','Cache-Control':'no-cache, no-transform','Connection':'keep-alive','X-Accel-Buffering':'no'});res.flushHeaders();res.write('retry: 1000\n\n');p.streams.add(res);p.connectedOnce=true;p.lastSeen=Date.now();send(p,'history',p.chatHistory||[]);send(p,'drawing',r.strokes);state(r);
    ...
    if(req.method==='POST'&&url.pathname==='/api'){
      ...
      if(b.action==='create'||b.action==='join'){
        ...
        if(r&&!p&&b.action==='resume'){
          p=[...r.roster.values()].find(q=>q.token===b.token&&!q.explicitLeave&&q.detachedAt&&Date.now()-q.detachedAt<=120000);
          if(p){if(r.players.length>=12)throw Error('This room is full.');r.players.push(p);r.emptySince=null;p.detachedAt=null;p.guessed=['draw','reveal'].includes(r.phase)&&r.guesses?.has(p.id)||false;
            if(!r.players.some(q=>q.id===r.host))r.host=p.id;
            if(['choose','draw','reveal'].includes(r.phase)&&!r.queue.includes(p.id)&&!r.drawnThisRound?.has(p.id))r.queue.push(p.id);
            chat(r,`${p.name} reconnected.`);state(r);
          }
        }
        if(!r||!p){res.writeHead(410,{'Content-Type':'application/json'});res.end(JSON.stringify({error:'Room expired. Create or join a new room.'}));return;}p.lastSeen=Date.now();
        if(r.phase==='draw'&&Date.now()>=r.deadline)reveal(r);
        if(b.action==='resume'){result={room:r.code,token:p.token};}
        else if(b.action==='leave'){removePlayer(r,p,true);}
          ...
          if(b.turn!==undefined&&b.turn!==r.turn)throw Error('This drawing has ended.');
          const input=b.strokes??[b.stroke];if(!Array.isArray(input)||!input.length||input.length>64||r.strokes.length+input.length>15000)throw Error('Invalid drawing batch');
          const batch=input.map(s=>{if(!s||!/^#[0-9a-f]{6}$/i.test(s.color)||!Array.isArray(s.points)||!s.points.length||s.points.length>100||s.points.some(pt=>!Number.isFinite(pt?.x)||!Number.isFinite(pt?.y)))throw Error('Invalid stroke');return{color:s.color,size:clamp(s.size,1,60),points:s.points.map(pt=>({x:clamp(pt.x,0,1000),y:clamp(pt.y,0,700)}))};});
          for(const stroke of batch){r.strokes.push(stroke);r.players.filter(q=>q!==p).forEach(q=>send(q,'stroke',stroke));}
    ...
    const files={'/':'index.html','/app.js':'app.js','/style.css':'style.css','/rules.mjs':'rules.mjs','/live-connection.mjs':'live-connection.mjs'},file=files[url.pathname];if(!file){res.writeHead(404);res.end('Not found');return;}
